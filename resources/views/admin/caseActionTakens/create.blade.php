...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.caseActionTaken.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.case-action-takens.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('action_type') ? 'is-invalid' : '' }}" name="action_type_id" id="action_type_id" required>
                    @foreach($action_types as $id => $action_type)
                        <option value="{{ $id }}" {{ old('action_type_id') == $id ? 'selected' : '' }}>{{ $action_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('action_desc') ? 'is-invalid' : '' }}" name="action_desc" id="action_desc">{{ old('action_desc') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('action_date') ? 'is-invalid' : '' }}" type="text" name="action_date" id="action_date" value="{{ old('action_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('action_notes') ? 'is-invalid' : '' }}" type="text" name="action_notes" id="action_notes" value="{{ old('action_notes', '') }}">

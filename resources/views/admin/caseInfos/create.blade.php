...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.caseInfo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.case-infos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="case_ref_code">{{ trans('cruds.caseInfo.fields.case_ref_code') }}</label>
                <input class="form-control {{ $errors->has('case_ref_code') ? 'is-invalid' : '' }}" type="text" name="case_ref_code" id="case_ref_code" value="{{ old('case_ref_code', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('case_name') ? 'is-invalid' : '' }}" type="text" name="case_name" id="case_name" value="{{ old('case_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('case_date') ? 'is-invalid' : '' }}" type="text" name="case_date" id="case_date" value="{{ old('case_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('report_type') ? 'is-invalid' : '' }}" name="report_type_id" id="report_type_id">
                    @foreach($report_types as $id => $report_type)
                        <option value="{{ $id }}" {{ old('report_type_id') == $id ? 'selected' : '' }}>{{ $report_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('report_num') ? 'is-invalid' : '' }}" type="text" name="report_num" id="report_num" value="{{ old('report_num', '') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('case_desc') ? 'is-invalid' : '' }}" name="case_desc" id="case_desc">{{ old('case_desc') }}</textarea>

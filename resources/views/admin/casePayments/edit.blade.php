...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.casePayment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.case-payments.update", [$casePayment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('case') ? 'is-invalid' : '' }}" name="case_id" id="case_id" required>
                    @foreach($cases as $id => $case)
                        <option value="{{ $id }}" {{ (old('case_id') ? old('case_id') : $casePayment->case->id ?? '') == $id ? 'selected' : '' }}>{{ $case }}</option>
                    @endforeach
                </select>
                @if($errors->has('case'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('payment_date') ? 'is-invalid' : '' }}" type="text" name="payment_date" id="payment_date" value="{{ old('payment_date', $casePayment->payment_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_amt') ? 'is-invalid' : '' }}" type="number" name="payment_amt" id="payment_amt" value="{{ old('payment_amt', $casePayment->payment_amt) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crncy') ? 'is-invalid' : '' }}" name="crncy_id" id="crncy_id" required>
                    @foreach($crncies as $id => $crncy)
                        <option value="{{ $id }}" {{ (old('crncy_id') ? old('crncy_id') : $casePayment->crncy->id ?? '') == $id ? 'selected' : '' }}>{{ $crncy }}</option>
                    @endforeach
                </select>
                @if($errors->has('crncy'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_ref_code') ? 'is-invalid' : '' }}" type="text" name="payment_ref_code" id="payment_ref_code" value="{{ old('payment_ref_code', $casePayment->payment_ref_code) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payment_notes') ? 'is-invalid' : '' }}" type="text" name="payment_notes" id="payment_notes" value="{{ old('payment_notes', $casePayment->payment_notes) }}">

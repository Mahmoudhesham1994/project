...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.outDocument.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.out-documents.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('doc') ? 'is-invalid' : '' }}" name="doc_id" id="doc_id">
                    @foreach($docs as $id => $doc)
                        <option value="{{ $id }}" {{ old('doc_id') == $id ? 'selected' : '' }}>{{ $doc }}</option>
                    @endforeach
                </select>
                @if($errors->has('doc'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('out_date') ? 'is-invalid' : '' }}" type="text" name="out_date" id="out_date" value="{{ old('out_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('return_date') ? 'is-invalid' : '' }}" type="text" name="return_date" id="return_date" value="{{ old('return_date') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('borrower') ? 'is-invalid' : '' }}" name="borrower_id" id="borrower_id">
                    @foreach($borrowers as $id => $borrower)
                        <option value="{{ $id }}" {{ old('borrower_id') == $id ? 'selected' : '' }}>{{ $borrower }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('out_notes') ? 'is-invalid' : '' }}" type="text" name="out_notes" id="out_notes" value="{{ old('out_notes', '') }}">
